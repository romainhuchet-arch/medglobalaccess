"""Liste des départements (code COG → nom), pour l'export et les sélecteurs."""
...
def nom(code: str) -> str:
    return DEPARTEMENTS.get(code, f"Département {code}")
...
def selection(valeur: str) -> list[str]:
    """'all' / 'tous' → tous ; '44,29 35' → ['44', '29', '35']."""
    if valeur.strip().lower() in {"all", "tous", "*"}:
        return list(DEPARTEMENTS)
    return [normaliser(c) for c in valeur.replace(",", " ").split() if c.strip()]
